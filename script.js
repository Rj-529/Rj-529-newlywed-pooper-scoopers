(function(){
  ...
  function showStep(n){var steps=document.querySelectorAll('#quote .qstep');for(var i=0;i<steps.length;i++){steps[i].hidden=(parseInt(steps[i].getAttribute('data-step'),10)!==n)}}
  function updateEstimate(){var rate=state.plan==='weekly'?24:20;var extraDogs=state.dogs-1;$('qest-amount').innerHTML='$'+rate+'<span class="qest-per">/visit</span>';$('qest-month').textContent=extraDogs>0?'+$'+(extraDogs*4)+'/week for '+extraDogs+' extra dog'+(extraDogs>1?'s':''):'1 dog'}
  var zipBtn=$('qzip-btn');if(zipBtn){zipBtn.addEventListener('click',function(){var zip=$('qzip').value.trim();if(!/^[0-9]{5}$/.test(zip)){$('qzip').focus();return}state.zip=zip;$('qzip-msg').textContent=SOUTH_TAMPA_ZIPS.indexOf(zip)>-1?"Good news — we're already picking up in "+zip+"!":"We'll double check "+zip+" is on our route — here's a quote either way:";updateEstimate();showStep(2)})}
  var toggles=document.querySelectorAll('.qtoggle-btn');for(var t=0;t<toggles.length;t++){toggles[t].addEventListener('click',function(){for(var j=0;j<toggles.length;j++){toggles[j].classList.remove('active')}this.classList.add('active');state.plan=this.getAttribute('data-plan');updateEstimate()})}
